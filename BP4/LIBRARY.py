...
class Parser:
    ...
    @staticmethod
    def get_attr(soup: Union[BeautifulSoup, Tag], key: str) -> Optional[str]:
        """
        주어진 속성 키의 값을 BeautifulSoup 객체에서 반환합니다.

        :param BeautifulSoup soup: 속성을 가져올 BeautifulSoup 객체.
        :param str key: 값을 가져올 속성 키.
        :return: 속성 값이 있으면 반환하고, 없으면 None을 반환합니다.
        """
        if not soup.has_attr(key):
            return None

        return soup.__getitem__(key=key)